export default function sketch(p){
  ...
  function ponto(x,y,p,escala)
  {
      ...
      p.push();
      p.translate(x,y);
      p.ellipse(0,0,7/escala,7/escala);
  ...
  function calcularFs()
  {
      fs = 1/Math.sqrt((pex**2-pex*pey+pey**2))*(ensaios.trac);
  ...
  p.draw = () => 
  {
      ...
      p.translate(0,0)
      p.rotate(Math.PI/4);
      p.ellipse(0,0,ensaios.trac*1.63,ensaios.trac*Math.sqrt(2)*2);
      
      p.stroke(fsfill)
      p.strokeWeight(2/escala);
      p.fill(0,0,0,0);
      p.ellipse(0,0,ensaios.trac*1.63/fs,ensaios.trac*Math.sqrt(2)*2/fs);
      p.rectMode(p.CENTER);
      p.pop()
      p.beginShape();
